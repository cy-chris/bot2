		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '❌ Este comando só pode ser usado pelo mestre chris ❌',
			...
			const ownerNumber = ["5599992241284@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'essa e a lista de numros bloqueados :\n'
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie imagens com a legenda ${prefix}sticker ou as tags de imagem que foram enviadas`)
					}
					break
				case 'gtts':
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'toma sua loli ai, lolicon do krl'})
					})
					break
				case 'nsfwloli':
					if (!isNsfw) return reply('❌ *NAO* ❌')
					loli.getNSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERRO* ❌')
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Não faça disso um material comli, tio'})
					})
					break
				case 'hilih':
				...
				case 'url2img':
					...
					if (args.length < 1) return reply('Hum que tipo?')
        ...
        case 'promote':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('O sucesso desabilita o modo simi neste grupo✔️')
					} else {
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('O recurso de boas-vindas foi ativado com sucesso neste grupo ✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Sucesso ao desativar o recurso de boas-vindas neste grupo✔️')
					} else {
						reply('1 para habilitar, 0 para desabilitar')
					}
                                      break
				case 'clone':
					...
					try {
						...
						mentions(`Foto do perfil Atualizada com sucesso usando uma foto do perfil @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Gagal om')
					}
					break
